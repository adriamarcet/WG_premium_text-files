--<< Textos del tutorial >>--

local tableText = {

	blancas = {
		--<< START translate >>--
		btnSetSalir     = "Salir del tutorial",
		btnSetContinuar = "Continuar jugando",
		btnFinAcabar    = "Acabar",
		btnFinContinuar = "Continuar"
		--<< END translate >>--
	},

	negras = {
		--<< START translate // transform && for \n and remove line comments (--<<)when applied >>--
		"¡Bienvenido al País de las Maravillas!\nSoy Alicia, tu guía en Wonder Gardens. Te enseñaré todo lo que hay que saber para jugar…\n¡y ganar!",
		"¡Como enviado de una de las 4 reinas, tu misión es pintar todas las rosas con su color!",
		"¡Empieza la partida! En esta partida eres el jugador ROJO, y en este caso el último en jugar...",
		"¡Te toca! Coloca tu loseta tocando en la posición que te indico o arrastrándola con el dedo.",
		"Ahora pulsa sobre la loseta para hacerla rotar y que haya 2 porciones rojas en el rosal. (Solo rotará de manera que coincidan sus caminos con los de las losetas contiguas)" ,
		"Ahora pulsa el botón \"OK\" para aceptar y así completar el rosal.",
		"¡Bien hecho! Uno de tus soldados se moverá hasta la loseta que acabas de colocar." ,
		"Te toca otra vez. ¡Es hora de anotar un punto! Al completar un rosal, si hay una mayoría de porciones de tu color ganarás 1 punto.",
		"Al completarse un rosal, se volverá del color que predomine y ese jugador anotará 1 punto. Pero desaparecerá si hay un empate entre los colores.",
		"¡Coloca y gira la nueva loseta en la posición indicada para completar el rosal y anotar un punto!",
		"¡Genial! ¡Has anotado tu primer punto! Ganará el primero en conseguir 7 puntos.\n¡Pero vigila! Al mismo tiempo, podrías hacer que los otros jugadores anoten puntos.",
		"¡Tu turno otra vez! Colocar tu nueva loseta aquí sería ideal, pero los soldados rivales te bloquean el camino.\nSólo podrás colocar losetas allí donde tus soldados puedan desplazarse.",
		"Pero hay una manera de colocar la loseta ahí, ¡Poner tu segundo soldado en juego!\nDispones de 2 soldados durante toda la partida, así que vamos a añadirlo.",
		"Pulsa el icono de soldado para añadirlo a la loseta que te ha tocado este turno.",
		"Ahora puedes colocar la loseta en muchos más sitios y evitar así el bloqueo de los soldados rivales.\n Cóloca la loseta en la posición indicada, rótala, y acepta.",
		"¡MUY BIEN!\nCon los 2 soldados en juego, un soldado siempre irá hasta la loseta que coloques, y podrás mover el segundo a donde quieras para bloquear a tus rivales.",
		"Ahora pulsa en la posición indicada del camino y el soldado se moverá hasta allí. A continuación pulsa el botón \"OK\".",
		"¡Nos han bloqueado! La única manera de sacar ese soldado de ahí es recuperándolo del tablero.\nSi estás bloqueado, al descartar una loseta dentro del sombrero de descarte, recuperarás 1 de tus soldados del tablero.",
		"Arrastra la loseta hacia la esquina inferior derecha de la pantalla y colócala dentro del sombrero que aparecerá. Luego pulsa el botón \"OK\" para confirmar.",
		"Elige cual de los dos soldados quieres recuperar.",
		"¡Muy bien! Como siempre escoge el camino ideal para tu otro soldado y termina el turno pulsando el botón \"OK\".",
		"¡Vuelve a ser tu turno! Es hora de usar tu loseta mágica! Una vez por partida puedes convertir tu loseta actual en una loseta mágica con las 4 porciones de tu color.",
		"Pulsa el botón de tu símbolo de corazón para activarlo, y después colócala en la posición indicada.",
		"¡Esto es todo lo que necesitas saber para jugar!\nDiviértete jugando con tus amigos y consigue sombreros o armas con las Conejomonedas.\nPuedes visitar la Tienda para personalizar a tus soldados.\n¡Hasta Pronto!",
		"Has terminado el tutorial, ¿quieres continuar la partida hasta los 7 puntos?",
		--<< END translate >>--
		}
	}

return tableText